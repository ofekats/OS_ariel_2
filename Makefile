...
STSHELL_OBJS = stshell.o

CODEC_A_LIB = libcodecA.so
CODEC_B_LIB = libcodecB.so

all: libcodecA.so libcodecB.so encode decode cmp copy stshell
...
stshell: $(STSHELL_OBJS)
	$(CC) -o $@ $^

clean:
	rm -f libcodecA.so libcodecB.so cmp copy encode decode stshell *.o
